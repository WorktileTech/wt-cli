...
module.exports = function (port) {
    ...
    const startUpgrade = function*(scripts, images, services, upgradeVersions, registryUsername, registryPassword, configStr) {
        let bashResults = [];

        // 1. exec pre script
        const preScript = replaceScriptArgs(scripts.preScript, {images: images, registryUsername: registryUsername, registryPassword: registryPassword});
        bashResults.push(yield exec(preScript));

        // 2. exec config upgrade script ::TODO
        if (configStr) {
            const configTempFilePath = path.resolve(__dirname, `../config/temp-config.js`);
            fs.writeFileSync(configTempFilePath, configStr, 'utf-8');
            const configScript = replaceScriptArgs(scripts.configScript, {configFilePath: configTempFilePath});
            yield exec(configScript);
            fs.unlinkSync(configTempFilePath);
        }
    ...
    router.post('/api/upgrade', verifyTokenMiddleware, function *(next) {
        ...
        try {
            if (!body.version || !body.upgradeVersions || !body.images || !body.services || !body.registryUsername || !body.registryPassword) {
                ...
                //开始执行升级程序
                let bashResults = yield startUpgrade(scripts, body.images, body.services, body.upgradeVersions, body.registryUsername, body.registryPassword,body.config);
