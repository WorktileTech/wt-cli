...
module.exports = function (port) {
    ...
    const startUpgrade = function*(scripts, images, services, upgradeVersions) {
        let bashResults = [];

        // 1. exec pre script
        const preScript = replaceScriptArgs(scripts.preScript, {images: images});
    ...
    router.post('/api/upgrade', verifyTokenMiddleware, function *(next) {
        ...
        try {
            ...
            //开始执行升级程序
            let bashResults = yield startUpgrade(scripts, body.images, body.services, body.upgradeVersions);
